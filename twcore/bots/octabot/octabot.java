...
public class octabot extends SubspaceBot {
	...
	public void storeGameResult( int team, int jackpot ) {
		...
		while( it.hasNext() ) {
			...
			OctaPlayer p = (OctaPlayer)players.get( name );
			try {
				m_botAction.SQLBackgroundQuery( "website", name, p.getQueryString( getUserId( name ), id ) );
			} catch (Exception e) {
	...
	public int getUserId( String name ) {
		...
		try {
			ResultSet result = m_botAction.SQLQuery( "website", "SELECT fnUserID FROM tblUser WHERE fcUserName = '"+Tools.addSlashesToString(name)+"'" );
			if( result.next() )
