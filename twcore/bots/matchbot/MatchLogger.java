...
public class MatchLogger {
    ...
    public void logEvent(Message event) {
        String cType = "",
               cName = m_botAction.getPlayerName( event.getPlayerID() ),
               m = event.getMessage();
        ...
        if (i == Message.ARENA_MESSAGE) {
            ...
            if (m.startsWith("C2S") || m.startsWith("S2C") || m.startsWith("IP:") || m.startsWith("Ping:")
                || m.startsWith("LOSS:")|| m.startsWith("TIME:") || m.startsWith("Bytes/Sec") || m.startsWith("Player Score Reset"))
                cType = "info";
        };
        if (i == Message.PUBLIC_MESSAGE) cType = "public";
        if (i == Message.TEAM_MESSAGE) cType = "team";
        if (i == Message.OPPOSING_TEAM_MESSAGE) cType = "opposing team";
        if (i == Message.PRIVATE_MESSAGE) cType = "private";
        if (i == Message.CHAT_MESSAGE) cType = "chat " + event.getChatNumber();
    ...
    public void logEvent(ScoreReset event) {
        String cName = m_botAction.getPlayerName( event.getPlayerID());
        if (event.getPlayerID() == -1) cName = "everybody";
    ...
    public void createLogRecord(String cMessageType, String cMessage, String cPlayer) {
        if (m_active) {
            try {
                m_botAction.SQLBackgroundQuery("local", "MatchLoggerAddRec", "INSERT INTO tblMatchLog(fnMatchID, fcMessageType, fcMessage, fcPlayer, fnOrder)" +
                            "VALUES("+m_fnMatchID+", '"+Tools.addSlashesToString(cMessageType)+"', '"+Tools.addSlashesToString(cMessage)+"', " +
                            "'"+Tools.addSlashesToString(cPlayer)+"', " + m_fnOrder + "  )");
                m_fnOrder++;
            } catch (Exception e) {
