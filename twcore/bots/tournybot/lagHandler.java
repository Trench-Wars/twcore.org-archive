...
public class lagHandler {
	...
	public void startLagRequest(String pName, String req, boolean sil, boolean bot) {
		...
		if (m_botAction.getFuzzyPlayerName(pName) == null) {
			...
			m_botAction.sendUnfilteredPrivateMessage(pName, "*info");
		}
	}
	...
	public void spamLagInfo(boolean tI, boolean present) {
		...
		if (present) {
			...
			if (!tI) {
				String lag[] = {
					playerName + ": PING Cur: " + currentPing + "ms Ave: " + averagePing + "ms Low: " + lowPing + "ms Hi: " + highPing + "ms PLOSS S2C: " + s2C + "% C2S: " + c2S + "% S2CWeapons: " + s2CWeapons + "%",
					Tools.formatString("", playerName.length(), "-") + "  SLOW S2C: " + s2CSlowPercent + "% C2S: " + c2SSlowPercent + "% NO SPIKE INFO WAS RETRIEVED"
				};
				if (!botR) { m_botAction.privateMessageSpam(requester, lag); }
			} else {
				String lag[] = {
					playerName + ": PING Cur: " + currentPing + "ms Ave: " + averagePing + "ms Low: " + lowPing + "ms Hi: " + highPing + "ms PLOSS S2C: " + s2C + "% C2S: " + c2S + "% S2CWeapons: " + s2CWeapons + "%",
					Tools.formatString("", playerName.length(), "-") + "  SLOW S2C: " + s2CSlowPercent + "% C2S: " + c2SSlowPercent + "% SPIKE Med: " + medF.format(spikeSD) + " Count: " + numSpikes
				};
				if (!botR) { m_botAction.privateMessageSpam(requester, lag); }
			}
			...
			if (!botR) { m_botAction.sendPrivateMessage(requester, "Could not find '" + playerName + "' on the arena."); }
		}
	...
	class lagRequest {
		...
		public boolean getBot() { return botR; };
	};
}