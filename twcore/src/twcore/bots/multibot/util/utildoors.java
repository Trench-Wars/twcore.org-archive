...
public class utildoors extends MultiUtil {
    ...
    public void handleCommand(String sender, String command) {
        ...
        if (command.startsWith("!doormode ")) {
            doDoorCmd(sender, command.substring(10));
        }
        ...
        if (command.startsWith("!adddoor ")) {
            doAddCmd(sender, command.substring(9));
        }

        if (command.startsWith("!doorlist")) {
            doListCmd(sender);
        }

        if (command.startsWith("!doordel ")) {
            doDelCmd(sender, command.substring(9));
        }

        if (command.startsWith("!doorsoff")) {
            doClearCmd(sender);
        }
    }
    ...
    private class DoorTimerTask extends TimerTask {
        ...
        public boolean exists(int taskNumber) {
            if (doorTaskList.containsKey(taskNumber)) {
                return true;
            }
            else {
                return false;
    ...
    public class DoorTask {
        ...
        public int toggle(int curDoorState) {
            if (isOn) {
                ...
                for (x = 0; x < doors.length; x++) {
                    curDoorState = openDoor(curDoorState, doors[x]);
                }
            } else {
                trigger = onTime;
                isOn = true;
                ...
                for (x = 0; x < doors.length; x++) {
                    curDoorState = closeDoor(curDoorState, doors[x]);
                }
            }
