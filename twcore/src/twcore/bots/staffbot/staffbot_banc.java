...
public class staffbot_banc extends Module {
    ...
    final String[] helpER = {
            ...
            /* " Arguments:", "             -player='<..>'     - Specifies player name",
    ...
    final String[] helpBancOp = { "---------------------[ BanC: Operator ]----------------------",
            " !addop                            - Adds a Banc Operator",
            " !removeop                         - Removes a Banc Operator and adds them to Revoked list",
            " !listops                          - Displays all Banc Operators and Banc Revoked Operators",
            " !deleteop                         - If removed from staff, use this.",
    ...
    public void initializeModule() {
        ...
        psActiveBanCs =
                m_botAction.createPreparedStatement(botsDatabase, uniqueConnectionID,
                        "SELECT * FROM tblBanc WHERE DATE_ADD(fdCreated, INTERVAL fnDuration MINUTE) > NOW() AND fbLifted = 0 OR fnDuration = 0");
        ...
        psLookupIPMID =
                m_botAction
                        .createPreparedStatement(
                                trenchDatabase,
                                uniqueConnectionID,
                                "SELECT fcIpString, fnMachineId FROM tblAlias INNER JOIN tblUser ON tblAlias.fnUserID = tblUser.fnUserID WHERE fcUserName = ? ORDER BY fdUpdated DESC LIMIT 0,1");
    ...
    private boolean sendBanCs(String stafferName, String name, int limit) throws SQLException {
         ...
         * String query;
         * 
         * if(limit == -1) query = "SELECT * from tblBanc WHERE fcUsername = '"+name+"' ORDER BY fdCreated ASC"; else query =
         * "SELECT * from tblBanc WHERE fcUsername = '"+name+"' ORDER BY fdCreated ASC "+" LIMIT 0,"+limit;
         * 
         * ResultSet rs = m_botAction.SQLQuery(botsDatabase, query);
         * 
         * if(rs == null ) m_botAction.sendSmartPrivateMessage(stafferName, "No banCs made on the player "+name);
         * 
         * else{ String result = "BanCs: ";
         * 
         * while(rs.next()){ result += Tools.formatString(rs.getString("fcUsername"), 10); result += Tools.formatString(rs.getString("fcType"), 10);
         * 
         ...
         * 
         * String MID = rs.getString("fcMID"); if(MID == null) MID = "(UNKNOWN)";
         * 
         * result += "MID: "+Tools.formatString(MID, 10);
         * 
         * int duration = rs.getInt("fnDuration"); boolean isDay = duration >= 1440? true:false;
         * 
         * if(isDay){ duration = (duration/60)/24; result += Tools.formatString(" Duration: "+duration+" days", 19); } else result +=
         * Tools.formatString(" Duration: "+duration+" mins", 19);
         * 
         * result += Tools.formatString(" by: " + rs.getString("fcStaffer"), 17); String comments = rs.getString("fcComment");
         * 
         * if(comments == null) comments = "No Comments";
         * 
         * list.add(result); list.add(comments); } String strSpam[] = list.toArray(new String[list.size()]);
    ...
    private boolean sendWarnings(String stafferName, String name, int limit) throws SQLException {
        ...
        if (limit == 0) {
            m_botAction.sendRemotePrivateMessage(stafferName, "There are " + expiredWarnings.size()
    ...
    private void sendAltNicks(String stafferName, String playerName, int limitBan, int limitWarn) {
        ...
        try {
            String ipResults =
                    getSubQueryResultString(
                            "SELECT DISTINCT(fnIP) " + "FROM `tblAlias` INNER JOIN `tblUser` ON `tblAlias`.fnUserID = `tblUser`.fnUserID "
                                    + "WHERE fcUserName = '" + Tools.addSlashesToString(playerName) + "'", "fnIP");

            String midResults =
                    getSubQueryResultString(
                            "SELECT DISTINCT(fnMachineId) " + "FROM `tblAlias` INNER JOIN `tblUser` ON `tblAlias`.fnUserID = `tblUser`.fnUserID "
                                    + "WHERE fcUserName = '" + Tools.addSlashesToString(playerName) + "'", "fnMachineId");

            String queryString =
                    "SELECT * " + "FROM `tblAlias` INNER JOIN `tblUser` ON `tblAlias`.fnUserID = `tblUser`.fnUserID " + "WHERE fnIP IN " + ipResults
            ...
            if (ipResults != null || midResults != null) {
                ...
                while (i.hasNext()) {
                    ...
                    ResultSet w =
                            m_botAction.SQLQuery(this.trenchDatabase, "SELECT * FROM tblWarnings WHERE name = '" + Tools.addSlashesToString(s)
                    ...
                    ResultSet b =
                            m_botAction.SQLQuery(this.botsDatabase, "SELECT * FROM tblBanc WHERE fcUsername = '" + Tools.addSlashesToString(s)
        ...
        if (IPCBANC.equals(event.getChannel()) && event.getSenderName().toLowerCase().startsWith("tw-guard")) {
            ...
            if (command.equals("BANC PUBBOT INIT")) {
                ...
                if (banc != null && banc.isNotification()) {
                    ...
                    m_botAction.sendChatMessage("Player '" + command.substring(8) + "' has been (re)silenced. (BanC #?)");
                }
                ...
                if (banc != null && banc.isNotification()) {
                    ...
                    m_botAction.sendChatMessage("Player '" + command.substring(5) + "' has been (re)locked in spectator. (BanC #?)");
                }// SPEC PLAYER
                 // 012345
                ...
                if (banc != null && banc.isNotification()) {
                    ...
                    m_botAction.sendChatMessage("Player '" + command.substring(10) + "' has been (re)superlocked in spectator. (BanC #?)");
                }
    ...
    private void cmdBanAccess(String name, String parameters) {
         ...
         * 
         * ER MOD SMOD SYSOP Silence time/mins 10 60 240 none Speclock time/mins 30 60 120 none Auto-kick time/mins n/a 30 60 none */
    ...
    private void cmdSilenceSpecKick(String name, String message) {
        ...
        /*if( !Tools.isAllDigits(timeStr) && !timeStr.contains("d")){//|| !Tools.isAllDigits(timeStr) ) { m_botAction.sendRemotePrivateMessage(name,
         * "Syntax error. Please specify <playername>:<time/mins> or PM !help for more information."); return; } else */if (timeStr.length() > 6) {
        ...
        if (time > BANC_MAX_DURATION) {
            m_botAction.sendRemotePrivateMessage(name, "The maximum amount of minutes for a BanC is " + BANC_MAX_DURATION
        ...
        if (getBanCAccessDurationLimit(bancType, opList.getAccessLevel(name)) != 0
            ...
            m_botAction.sendRemotePrivateMessage(name, "The duration has been changed to the maximum duration of your access level: " + time
                    + " mins.");
            timeToTell = 7;
        }
        ...
        if (time >= 24 * 60 * 7 && timeToTell > 0) {
            ...
            m_botAction.sendRemotePrivateMessage(name, "BanC #" + banc.id + ": " + bancName + " on '" + target + "' for " + timeToTell + " days("
                    + time + " mins) initiated.");
        } else if (time > 0) {
            m_botAction.sendChatMessage(name + " initiated an " + bancName + " on '" + target + "' for " + time + " minutes.");
            m_botAction.sendRemotePrivateMessage(name, "BanC #" + banc.id + ": " + bancName + " on '" + target + "' for " + time
                    + " minutes initiated.");
        } else {
            m_botAction.sendChatMessage(name + " initiated an infinite/permanent " + bancName + " on '" + target + "'.");
            m_botAction.sendRemotePrivateMessage(name, "BanC #" + banc.id + ": " + bancName + " on '" + target
    ...
    private void cmdListBan(String name, String parameters, boolean showLBHelp, boolean twoWeeks) {
        ...
        /*!listban [arg] [#id] [count] - Shows last 10/[count] BanCs or info about BanC with <id>. Arguments see below. !listban <player>:[#] - Shows
         * the last [#]/10 BanCs applied on <player> Arguments: -player='<..>' - Specifies player name -d=# - Specifies duration in minutes -a=<...> -
         * Specifies access requirement, options; mod / smod / sysop -ip=<#.#.#.#> -ir - Specifies IP or remove IP (-ir) so banc is not matched by IP
         * -mid=# -mr - Specifies MID or remove MID (-mr) so banc is not matched by MID -notif=<yes/no> - Specifies wether a notification is sent on
         * staff chat */
        ...
        try {
            ...
            if (sqlWhere.contains("fnID")) {
                sqlQuery =
                        "SELECT (DATE_ADD(fdCreated, INTERVAL fnDuration MINUTE) > NOW() OR fnDuration = 0) AS active, fnID, fcType, fcUsername, fcIP, fcMID, fcMinAccess, fnDuration, fcStaffer, fcComment, fbNotification, fdCreated, fbLifted FROM tblBanc WHERE "
                                + sqlWhere + " LIMIT 0,1";
                ...
                sqlQuery =
                        "SELECT (DATE_ADD(fdCreated, INTERVAL fnDuration MINUTE) > NOW() OR fnDuration = 0) AS active, fnID, fcType, fcUsername, fcIP, fcMID, fcMinAccess, fnDuration, fcStaffer, fdCreated, fbLifted FROM tblBanc "
            ...
            m_botAction.sendRemotePrivateMessage(name,
                    "A problem occured while retrieving ban listing from the database. Please try again or report the problem.");
    ...
    private void cmdChangeBan(String name, String parameters) {
        ...
        /*!changeban <#id> <arguments> - Changes banc with <id>. Arguments see below. Arguments: -player=<..> - Specifies player name -d=# - Specifies
         * duration in minutes -a=<...> - Specifies access requirement, options; mod / smod / sysop -ip=<#.#.#.#> -ir - Specifies IP or remove IP
         * (-ir) so banc is not matched by IP -mid=# -mr - Specifies MID or remove MID (-mr) so banc is not matched by MID -notif=<yes/no> - Specifies
         * wether a notification is sent on staff chat */
                ...
                if (!playerArgument) {
                    ...
                    if (argument.startsWith("-a=")) {
                        ...
                        if ((MINACCESS_ER.equalsIgnoreCase(accessRequirement) && !opList.isER(name))
                            ...
                            m_botAction.sendRemotePrivateMessage(name,
                                    "You can't set the access requirement higher then your own access. (Argument ignored.)");
        ...
        if (sqlSet.isEmpty()) {
            // No arguments
            m_botAction.sendRemotePrivateMessage(name,
                    "Syntax error (no arguments specified). Please specify #id and arguments. For more information, PM !help.");
            return;
        }
        ...
        try {
            ...
            m_botAction.sendRemotePrivateMessage(name,
                    "A problem occured while modifying the ban in the database. Please try again or report the problem.");
    ...
    private void doForceDBConnection(String name) {
        try {
            ...
            m_botAction.sendPrivateMessage(name,
                    "I had a problem to force the DB Connection, try again in some minutes. StackTrace:" + " " + e.toString());
            e.printStackTrace();
        }
    }
    ...
    private void cmdLiftban(String name, String message) {
        ...
        try {
            ...
            m_botAction.sendRemotePrivateMessage(name,
                    "A problem occured while deleting the banc from the database. Please try again or report the problem.");
            Tools.printStackTrace("SQLException while modifying the database", sqle);
        }
    }
    ...
    private void cmdListActiveBanCs(String name) {
        for (BanC banc : activeBanCs) {
            m_botAction.sendRemotePrivateMessage(name,
                    "#" + banc.getId() + " " + banc.getType() + " " + banc.getDuration() + "mins on " + banc.getPlayername());
        }
    }
    ...
    private BanC lookupActiveBanC(BanCType bancType, String name) {
        for (BanC banc : this.activeBanCs) {
            if (banc.type.equals(bancType) && banc.playername.equalsIgnoreCase(name)) { return banc; }
        }
        return null;
    }
    ...
    private void dbAddBan(BanC banc) {
        ...
        try {
            // INSERT INTO tblBanc(fcType, fcUsername, fcIP, fcMID, fcMinAccess, fnDuration, fcStaffer, fdCreated) VALUES (?, ?, ?, ?, ?, ?, ?, ?,
            // NOW())
            // 1 2 3 4 5 6 7
