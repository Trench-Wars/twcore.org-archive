    ...
    */
    ...
    public void handleEvent( Message event ) {
        ...
        if(message != null && event.getMessageType() == Message.ARENA_MESSAGE) {
            ...
            } else if(message.startsWith("Ping:") && message.indexOf("HighPing:") > 0) {
                infoBuffer[1] = message;     // *info 2nd line
            } else if(message.startsWith("LOSS:") && message.indexOf("S2CWeapons:") > 0) {
                infoBuffer[2] = message;
            } else if(message.startsWith("S2C:") && message.indexOf("C2S:") > 0) {
                infoBuffer[3] = message;
            } else if(message.startsWith("C2S CURRENT: Slow:") && message.indexOf("TOTAL: Slow:") > 0) {
                infoBuffer[4] = message;
            } else if(message.startsWith("S2C CURRENT: Slow:") && message.indexOf("TOTAL: Slow:") > 0) {
                infoBuffer[5] = message;
            } else if(message.startsWith("TIME: Session:") && message.indexOf("Total:") > 0) {
                infoBuffer[6] = message;
            } else if(message.startsWith("Bytes/Sec:") && message.indexOf("ConnectType:") > 0 ) {
                infoBuffer[7] = message;
                processInfoBuffer(infoBuffer);
            } else if(message.indexOf("UserId:") > 0 && message.indexOf("Timer drift:") > 0) {
                // *einfo results
