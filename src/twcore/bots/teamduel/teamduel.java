...
import twcore.bots.teamduel.DBPlayerData;
import twcore.bots.teamduel.Duel;
...
public class teamduel extends SubspaceBot {
    ...
    HashSet<TeamInvite> invites = new HashSet<TeamInvite>();
    ...
    public void handleEvent(PlayerDeath event) {
        ...
        if (playing.containsKey(name)) {
            ...
            if (killer.equalsIgnoreCase(names[0]) || killer.equalsIgnoreCase(names[1])) {
                ...
                if (spawndx < s_spawnTime + s_extra) {
                    ...
                    if (duel.getPlayer(killer).getSpawns() >= s_spawnLimit) {
        ...
        if (playing.containsKey(name)) {
            ...
            coord = playing.get(name).getPlayer(name).getCoords();
            SpawnDelay warp = new SpawnDelay(name, coord[0], coord[1], System.currentTimeMillis());
            try {
                spawnDelays.put(name, warp);
                m_botAction.scheduleTask(warp, s_spawn * 1000);
    ...
    public void handleEvent(FrequencyShipChange event) {
        ...
        if (ship != 0) {
            if (!laggers.containsKey(name) && !player.isWarping() && !duel.isLocked()) {
                if (ship == 6) {
    ...
    public void do_showHelp(String name, String message) {
        ...
        if(leagueOps.containsKey(name.toLowerCase()) || leagueHeadOps.containsKey(name.toLowerCase()) || hiddenOps.containsKey(name.toLowerCase())) {
            String help2[] = {
                ...
                "| !aliasall <name>                   - Checks for names with the same IP/MID, IP, or MID.     |",
        ...
        if (!sql_banned(name)) {
            if (sql_enabledUser(name)) {
                try {
                    ...
                    "FROM tblDuel2League L JOIN tblDuel2Team T JOIN tblUser U JOIN tblUser U2 JOIN tblUser U3 " + 
    ...
    public void do_joinTeam(String name, String message) {
        ...
        while (!found && i.hasNext()) {
            invite = i.next();
            if (invite.invitee.equalsIgnoreCase(name) && invite.inviter.equalsIgnoreCase(message))
                found = true;
        }
        if (found) {
    ...
    public void do_setRules(String name, String message) {
        ...
        boolean nc = true;
        if (message.isEmpty()) {
            String query = "SELECT P.fnGameDeaths, P.fnNoCount FROM tblDuel2Player P JOIN tblUser U ON U.fnUserID = P.fnUserID WHERE P.fnEnabled = 1 AND U.fcUserName = '" + Tools.addSlashesToString(name) + "'";
            try {
        ...
        if (nc) {
            String query = "UPDATE tblDuel2Player SET fnGameDeaths = " + deaths + ", fnNoCount = 1 WHERE fnEnabled = 1 AND fnUserID = (SELECT fnUserID FROM tblUser WHERE fcUserName = '" + Tools.addSlashesToString(name) + "')";
            try {
            ...
            String query = "UPDATE tblDuel2Player SET fnGameDeaths = " + deaths + ", fnNoCount = 0 WHERE fnEnabled = 1 AND fnUserID = (SELECT fnUserID FROM tblUser WHERE fcUserName = '" + Tools.addSlashesToString(name) + "')";
            try {
                m_botAction.SQLQueryAndClose(mySQLHost, query);
    ...
    public void do_setNotPlaying(String name, String message) {
        if (notPlaying.containsKey(name)) {
            ...
            LinkedList<Integer> teams = notPlaying.remove(name);
            if (!teams.isEmpty()) {
    ...
    public void do_enableName(String name, String message) {
        ...
        try {
            ...
            ResultSet result = m_botAction.SQLQuery(mySQLHost, "SELECT fcUserName FROM tblDuel2Player WHERE fnEnabled = 1 AND (fcIP = '"
                    + IP + "' OR fnMID = '" + MID + "')");
    ...
    public void do_quitDivision(String name, String message) {
        ...
        if ((division > 0 && division < 6) || division == 7) {
            if (division == 7)
                division = 4;
            try {
                String query = "SELECT * FROM tblDuel2Team WHERE fnSeason = " + s_season + " " +
                ...
                while (teams.next()) {
                    ...
                    m_botAction.SQLQueryAndClose(mySQLHost, "UPDATE tblDuel2League SET fnStatus = 0 " +
                    		"WHERE fnSeason = " + s_season + 
                    		" AND fnStatus = 1 AND fnLeagueTypeID = " + division + " AND fnTeamID = " + team);
                    m_botAction.SQLQueryAndClose(mySQLHost, "UPDATE tblDuel2Team SET fnStatus = 0 " +
                            "WHERE fnSeason = " + s_season + 
                            " AND fnStatus = 1 AND fnLeagueTypeID = " + division + " AND fnTeamID = " + team);
                    m_botAction.SQLQueryAndClose(mySQLHost, "UPDATE tblDuel2Team SET fnStatus = 0 " +
                            "WHERE fnSeason = " + s_season + 
    ...
    public void do_showRank(String name, String message) {
        ...
        try {
            String query = "SELECT T.fnTeamID, T.fnLeagueTypeID, T.fnRating FROM tblDuel2Team T " + 
            ...
            ResultSet rank = m_botAction.SQLQuery(mySQLHost, query);
            while (rank.next()) {
                ranked = true;
                int teamID = rank.getInt("T.fnTeamID");
                int div = rank.getInt("T.fnLeagueTypeID");
                int rating = rank.getInt("T.fnRating");
                String team = "|";
        ...
        if (!ranked) {
            m_botAction.sendPrivateMessage(name, "No team information found for " + player);
        }
    }
    ...
    public void do_allowUser(String name, String message) {
        if (!leagueOps.containsKey(name.toLowerCase()) && !leagueHeadOps.containsKey(name.toLowerCase()) && !hiddenOps.containsKey(name.toLowerCase()))
            return;
        try {
            ResultSet result = m_botAction.SQLQuery(mySQLHost, "SELECT * FROM tblDuel2Player P JOIN tblUser U ON U.fnUserID = P.fnUserID WHERE U.fcUserName='" + Tools.addSlashesToString(message) + "'");
            if (result.next()) {
    ...
    public void do_sayBanned(String name, String message) {
        ...
        try {
            ResultSet results = m_botAction.SQLQuery(mySQLHost, "SELECT B.*, U.fcUserName FROM tblDuel2Ban B JOIN tblUser U ON U.fnUserID = B.fnUserID");
            while (results.next())
    ...
    public void do_reComment(String name, String message) {
        ...
        try {
            ...
            if (sql_banned(player)) {
                ResultSet getTime = m_botAction.SQLQuery(mySQLHost, "SELECT B.fcComment FROM tblDuel2Ban B JOIN tblUser U ON U.fnUserID = B.fnUserID WHERE U.fcUserName='"
                ...
                if (banOnDate.length() > 0) {
                    ...
                    m_botAction.SQLQueryAndClose(mySQLHost, "UPDATE tblDuel2Ban SET fcComment = '"
                            + comment
    ...
    public void do_getComment(String name, String message) {
        ...
        try {
            if (sql_banned(player)) {
                ResultSet results = m_botAction.SQLQuery(mySQLHost, "SELECT * FROM tblDuel2Ban WHERE fnUserID = (SELECT fnUserID FROM tblUser WHERE fcUserName = '"
                        + Tools.addSlashesToString(player) + "')");
                if (results.next())
                    m_botAction.sendPrivateMessage(name, "Ban comment: "
                            + results.getString("fcComment"));
    ...
    public void do_aliasCheck(String name, String message) {
        ...
        try {
            ResultSet result = m_botAction.SQLQuery(mySQLHost, "SELECT U.fcUserName FROM tblDuel2Player P JOIN tblUser U ON U.fnUserID = P.fnUserID WHERE P.fnEnabled = 1 AND P.fcIP = (SELECT P.fcIP FROM tblDuel2Player P JOIN tblUser U ON U.fnUserID = P.fnUserID WHERE U.fcUserName = '" + Tools.addSlashesToString(message) + "') AND P.fnMID = (SELECT P.fnMID FROM tblDuel2Player P JOIN tblUser U ON U.fnUserID = P.fnUserID WHERE U.fcUserName = '" + Tools.addSlashesToString(message) + "')");
            if (result.next()) {
                String extras = "";
                do {
                    extras += "" + result.getString("U.fcUserName") + ", ";
                } while (result.next());
                m_botAction.sendSmartPrivateMessage(name, "Aliases registered: " + extras.substring(0, extras.length() - 2));
                m_botAction.SQLClose(result);
                return;
            } else {
                String player = m_botAction.getFuzzyPlayerName(message);
    ...
    public void do_aliasCheckAll(String name, String message) {
        ...
        try {
            ...
            if (result.next()) {
                ...
                m_botAction.sendSmartPrivateMessage(name, "Aliases registered with the same IP and MID: " + extras.substring(0, extras.length() - 2));
                m_botAction.SQLClose(result);
            } else {
                String player = m_botAction.getFuzzyPlayerName(message);
                if (player == null) {
            ...
            result = m_botAction.SQLQuery(mySQLHost, "SELECT U.fcUserName FROM tblDuel2Player P JOIN tblUser U ON U.fnUserID = P.fnUserID WHERE P.fnEnabled = 1 AND P.fcIP = (SELECT P.fcIP FROM tblDuel2Player P JOIN tblUser U ON U.fnUserID = P.fnUserID WHERE U.fcUserName = '" + Tools.addSlashesToString(message) + "')");
            if (result.next()) {
                String extras = "";
                do {
                    extras += "" + result.getString("U.fcUserName") + ", ";
            ...
            result = m_botAction.SQLQuery(mySQLHost, "SELECT U.fcUserName FROM tblDuel2Player P JOIN tblUser U ON U.fnUserID = P.fnUserID WHERE P.fnEnabled = 1 AND P.fnMID = (SELECT P.fnMID FROM tblDuel2Player P JOIN tblUser U ON U.fnUserID = P.fnUserID WHERE U.fcUserName = '" + Tools.addSlashesToString(message) + "')");
            if (result.next()) {
                String extras = "";
                do {
                    extras += "" + result.getString("U.fcUserName") + ", ";
                } while (result.next());
                m_botAction.sendSmartPrivateMessage(name, "Aliases registered with the same MID: " + extras.substring(0, extras.length() - 2));
                m_botAction.SQLClose(result);
            }
            m_botAction.SQLClose(result);
            return;
    ...
    public int do_getUserID(String name) {
        DBPlayerData getID = new DBPlayerData(m_botAction, mySQLHost, name, false);
    ...
    */
    ...
    public void do_addPlayer(String name, String IP, String MID) {
        try {
            ResultSet result = m_botAction.SQLQuery(mySQLHost, "SELECT U.fcUserName FROM tblDuel2Player P JOIN tblUser U ON U.fnUserID = P.fnUserID WHERE fnEnabled = 1 AND fcIP = '"
                    + IP + "' AND P.fnMID = '" + MID + "')");
            if (!result.next()) {
                m_botAction.SQLQueryAndClose(mySQLHost, "INSERT INTO tblDuel2Player (`fnUserID`, `fcIP`, `fnMID`, `fnLag`, `fnLagCheckCount`, `fdLastPlayed`) " + 
                ...
                if (result.getString("U.fcUserName").equalsIgnoreCase(name)) {
                    ...
                    if (allowedNames.containsKey(name)
                            && aliasChecker.equals("")) {
                        m_botAction.SQLQuery(mySQLHost, "INSERT INTO tblDuel2Player (`fnUserID`, `fcIP`, `fnMID`, `fnLag`, `fnLagCheckCount`, `fdLastPlayed`) " + 
                                "SELECT fnUserID, '" + IP + "', '" + MID + "', 0, 0, NOW() FROM tblUser WHERE fcUserName = '" + Tools.addSlashesToString(name) + "'");
    ...
    public void do_checkArena(String name, String message) {
        if (message.startsWith("IP:")) {
            ...
            try {
                ResultSet result = m_botAction.SQLQuery(mySQLHost, "SELECT p.fnUserID, p.fnLagCheckCount, p.fnLag FROM tblDuel2Player p JOIN tblUser u ON u.fnUserID = p.fnUserID " +
                ...
                if (result != null) {
                    if (result.next()) {
                        ...
                        m_botAction.SQLQueryAndClose(mySQLHost, "UPDATE tblDuel2Player SET fnLag = " + average2 + ", fnLagCheckCount = fnLagCheckCount + 1 " +
    ...
    public void endDuel(Duel d, String[] winner, String[] loser, int winnerTeam, int loserTeam, int type) {
        ...
        m_botAction.sendUnfilteredPrivateMessage(d.getPlayerThree().getName(), "*lag");
        to = null;
        from = d.getPlayerThree().getName();
        ...
        m_botAction.spec(loser[0]);
        m_botAction.spec(loser[1]);
        m_botAction.shipReset(winner[0]);
        ...
        int winnerStreak = 0; 
        int winnerCurStreak = 0;
        int loserRatingBefore = 0;
        int winnerRatingBefore = 0;
        try {
            String query = "SELECT * FROM tblDuel2Team WHERE fnStatus = 1 AND fnSeason = " + s_season + " AND fnTeamID = " + loserTeam;
            ...
            query = "SELECT * FROM tblDuel2Team WHERE fnStatus = 1 AND fnSeason = " + s_season + " AND fnTeamID = " + winnerTeam;
        ...
        int winnerRatingAfter = (int) Math.round((winnerRatingBefore + 32*(1 - (1 / (1 + Math.pow(10, (loserRatingBefore - winnerRatingBefore) / 400))))));
        int loserRatingAfter = (int) Math.round((loserRatingBefore + 32*(0 - (1 / (1 + Math.pow(10, (winnerRatingBefore - loserRatingBefore) / 400))))));
        
        if (winnerRatingAfter < 0)
            winnerRatingAfter = 0;
        if (loserRatingAfter < 0)
            loserRatingAfter = 0;
        ...
        try {
            ...
            String query = "INSERT INTO `tblDuel2Match` (`fnSeason`, `fnLeagueTypeID`, `fnBoxType`, `fnWinnerScore`, `fnLoserScore`, `fnWinnerTeamID`, `fnLoserTeamID`, `fnWinner1Ship`, `fnWinner2Ship`, `fnLoser1Ship`, `fnLoser2Ship`, `fnCommentID`, `fnDeaths`, `fnDuration`, `fnWinnerRatingBefore`, `fnWinnerRatingAfter`, `fnLoserRatingBefore`, `fnLoserRatingAfter` ) VALUES (";
        ...
        try {
            m_botAction.SQLQueryAndClose(mySQLHost, "UPDATE tblDuel2Team SET fdLastPlayed = NOW() WHERE fnSeason = " + s_season + " AND fnStatus = 1 AND fnTeamID = " + winnerTeam + " AND fnLeagueTypeID = " + division);
            m_botAction.SQLQueryAndClose(mySQLHost, "UPDATE tblDuel2Team SET fdLastPlayed = NOW() WHERE fnSeason = " + s_season + " AND fnStatus = 1 AND fnTeamID = " + loserTeam + " AND fnLeagueTypeID = " + division);
            m_botAction.SQLQueryAndClose(mySQLHost, "UPDATE tblDuel2Player SET fdLastPlayed = NOW() WHERE (fnUserID = " + loserInfo[0].getUserID() + " OR fnUserID = " + loserInfo[1].getUserID() + ")");
            m_botAction.SQLQueryAndClose(mySQLHost, "UPDATE tblDuel2Player SET fdLastPlayed = NOW() WHERE (fnUserID = " + winnerInfo[0].getUserID() + " OR fnUserID = " + winnerInfo[1].getUserID() + ")");
    ...
    public void do_score(Duel duel) {
    ...
    public DuelPlayer sql_getPlayer(String name) {
        try {
            ResultSet result = m_botAction.SQLQuery(mySQLHost, "SELECT * FROM tblDuel2Player p JOIN tblUser u ON u.fnUserID = p.fnUserID WHERE u.fcUserName = '"
    ...
    public void sql_verifyRecord(int userId, int divison) {
        try {
            String query = "SELECT * FROM tblDuel2League WHERE fnUserID = " + userId + " AND fnSeason = " + s_season + " AND fnStatus = 1 AND fnLeagueTypeID = " + divison;
            ResultSet result = m_botAction.SQLQuery(mySQLHost, query);
            if (!result.next())
                m_botAction.SQLQueryAndClose(mySQLHost, "INSERT INTO tblDuel2League (fnUserID, fnSeason, fnStatus, fnLeagueTypeID) VALUES (" + userId + ", " + s_season + ", 1, " + divison + ")");
            m_botAction.SQLClose(result);
        } catch (Exception e) {
    ...
    public ArrayList<Integer> sql_getTeams(String name) {
        ...
        for (int i = 1; i <=5; i++)
            teams.add(i, -1);
        try {
            String query = "SELECT l.fnUserID, l.fnTeamID, l.fnLeagueTypeID FROM tblDuel2League l JOIN tblUser u ON u.fnUserID = l.fnUserID WHERE l.fnSeason = " + s_season + " AND l.fnStatus = 1 AND u.fcUserName = '" + Tools.addSlashesToString(name) + "' ORDER BY l.fnLeagueTypeID";
            ResultSet result = m_botAction.SQLQuery(mySQLHost, query);
            if (result.next()) {
    ...
    public int sql_getTeamDivision(int teamID) {
        int id = 0;
        try {
            String query = "SELECT fnLeagueTypeID FROM tblDuel2Team WHERE fnSeason = " + s_season + " AND fnStatus = 1 AND fnTeamID = " + teamID;
            ResultSet result = m_botAction.SQLQuery(mySQLHost, query);
            if (result.next()) {
    ...
    public String[] sql_getPartners(int teamID) {
        ...
        try {
            String query = "SELECT u.fcUserName FROM tblUser u JOIN tblDuel2Team t ON (t.fnUser1ID = u.fnUserID OR t.fnUser2ID = u.fnUserID) WHERE t.fnSeason = " + s_season + " AND t.fnStatus = 1 AND t.fnTeamID = " + teamID;
    ...
    public boolean sql_banned(String name) {
        try {
            ...
            ResultSet result = m_botAction.SQLQuery(mySQLHost, "SELECT * FROM tblDuel2Ban b JOIN tblUser u ON u.fnUserID = b.fnUserID WHERE u.fcUserName = '"
    ...
    public boolean sql_banPlayer(String name, String comment) {
        try {
            m_botAction.SQLQueryAndClose(mySQLHost, "INSERT INTO tblDuel2Ban (fnUserID, fcComment) SELECT fnUserID, '" + Tools.addSlashesToString(comment) + "' FROM tblUser WHERE fcUserName = '" + Tools.addSlashesToString(name) + "'");
            return true;
        } catch (Exception e) {
            return false;
        }
    ...
    public boolean sql_unbanPlayer(String name) {
        try {
            m_botAction.SQLQueryAndClose(mySQLHost, "DELETE FROM tblDuel2Ban WHERE fnUserID = (SELECT fnUserID FROM tblUser WHERE fcUserName = '" + Tools.addSlashesToString(name) + "')");
            return true;
        } catch (Exception e) {
            return false;
        }
    ...
    public boolean sql_enabledUser(String name) {
        ...
        try {
            String query = "SELECT p.fnUserID FROM tblDuel2Player p JOIN tblUser u ON u.fnUserID = p.fnUserID WHERE p.fnEnabled = 1 AND u.fcUserName = '"
                    + Tools.addSlashesToString(name) + "'";
            ResultSet result = m_botAction.SQLQuery(mySQLHost, query);
    ...
    public void sql_enableUser(String name) {
        try {
            String query = "UPDATE tblDuel2Player SET fnEnabled = 1 WHERE fnUserID = (SELECT fnUserID FROM tblUser WHERE fcUserName = '" + Tools.addSlashesToString(name) + "')";
            m_botAction.SQLQueryAndClose(mySQLHost, query);
        } catch (Exception e) {
    ...
    public void sql_disableUser(String name) {
        try {
            m_botAction.SQLQueryAndClose(mySQLHost, "UPDATE tblDuel2Player SET fnEnabled = 0 WHERE fnUserID = (SELECT fnUserID FROM tblUser WHERE fcUserName = '" + Tools.addSlashesToString(name) + "')");
            m_botAction.SQLQueryAndClose(mySQLHost, "UPDATE tblDuel2Team SET fnStatus = 0 WHERE fnSeason=" + s_season + " AND fnStatus = 1 AND (fnUser1ID = (SELECT fnUserID FROM tblUser WHERE fcUserName = '" + Tools.addSlashesToString(name) + "') OR fnUser2ID = (SELECT fnUserID FROM tblUser WHERE fcUserName = '" + Tools.addSlashesToString(name) + "'))");
            m_botAction.SQLQueryAndClose(mySQLHost, "UPDATE tblDuel2League SET fnStatus=0 WHERE fnSeason=" + s_season + " AND fnStatus = 1 AND fnUserID = (SELECT fnUserID FROM tblUser WHERE fcUserName = '" + Tools.addSlashesToString(name) + "')");           
        } catch (Exception e) {
    ...
    public boolean sql_divisionCheck(String name, String part, int div) {
        if ((div > 0 && div < 6) || div == 7) {
            if (div == 7)
                div = 4;
            try {
                ResultSet user = m_botAction.SQLQuery(mySQLHost, "SELECT fnTeamID, fnStatus FROM tblDuel2League WHERE fnSeason = " + s_season + " AND fnUserID = (SELECT fnUserID FROM tblUser WHERE fcUserName = '" + Tools.addSlashesToString(name) + "') AND fnLeagueTypeID = " + div + " AND fnStatus = 1");
                if (user.next()) {
                ...
                m_botAction.SQLClose(user);                
                user = m_botAction.SQLQuery(mySQLHost, "SELECT fnTeamID, fnStatus FROM tblDuel2League WHERE fnSeason = " + s_season + " AND fnUserID = (SELECT fnUserID FROM tblUser WHERE fcUserName = '" + Tools.addSlashesToString(part) + "') AND fnLeagueTypeID = " + div + " AND fnStatus = 1");
                if (user.next()) {
    ...
    public int sql_createTeam(String a, String b, int div) {
        ...
        String query = "INSERT INTO tblDuel2Team (fnSeason, fnLeagueTYpeID, fnUser1ID, fnUser2ID) SELECT " + s_season + ", " + div + ", p.fnUserID, p2.fnUserID FROM tblUser p, tblUser p2 WHERE p.fcUserName = '" + Tools.addSlashesToString(a) + "' AND p2.fcUserName = '" + Tools.addSlashesToString(b) + "'";
        try {
            m_botAction.SQLQueryAndClose(mySQLHost, query);
            ResultSet rs = m_botAction.SQLQuery(mySQLHost, "SELECT fnTeamID FROM tblDuel2Team WHERE fnSeason = " + s_season + " AND fnLeagueTypeID = " + div + " AND fnUser1ID = (SELECT fnUserID FROM tblUser WHERE fcUserName = '" +  Tools.addSlashesToString(a) + "') AND fnUser2ID = (SELECT fnUserID FROM tblUser WHERE fcUserName = '" + Tools.addSlashesToString(b) + "') AND fnStatus = 1");
            if (rs.next())
                id = rs.getInt("fnTeamID");
            m_botAction.SQLClose(rs);
            query = "INSERT INTO tblDuel2League (fnSeason, fnTeamID, fnUserID, fnLeagueTypeID) SELECT " + s_season + ", " + id + ", fnUserID, " + div + " FROM tblUser WHERE fcUserName = '" + Tools.addSlashesToString(a) + "'";
            m_botAction.SQLQueryAndClose(mySQLHost, query);
            query = "INSERT INTO tblDuel2League (fnSeason, fnTeamID, fnUserID, fnLeagueTypeID) SELECT " + s_season + ", " + id + ", fnUserID, " + div + " FROM tblUser WHERE fcUserName = '" + Tools.addSlashesToString(b) + "'";
            m_botAction.SQLQueryAndClose(mySQLHost, query);
        } catch (SQLException e) {
            e.printStackTrace();
        }        
    ...
    public void sql_storeUserLoss(String name, int userID, int teamID, int division, int kills, int deaths, int spawns, int spawned, int lagouts, int time) {
        ...
        try {
            String query = "UPDATE tblDuel2League SET fnLosses = fnLosses + 1, fnKills = fnKills + " + kills + 
    ...
    public void sql_storeUserWin(String name, int userID, int teamID, int division, int kills, int deaths, int spawns, int spawned, int lagouts, int time) {
        try {
            String query = "UPDATE tblDuel2League SET fnWins = fnWins + 1, fnKills = fnKills + " + kills + 
            ", fnDeaths = fnDeaths + " + deaths + ", fnTimePlayed = fnTimePlayed + " + time + 
            ", fnSpawns = fnSpawns + " + spawns + ", fnSpawned = fnSpawned + " + spawned + 
            ", fnLagouts = fnLagouts + " + lagouts + 
            " WHERE fnSeason = " + s_season + " AND fnStatus = 1 AND fnTeamID = " + teamID + " AND fnUserID = " + userID + " AND fnLeagueTypeID = " + division;
    ...
    public void sql_storeTeamWin(int teamID, int division, int winnerStreak, int winnerCurStreak, int wonAgainst, int rating, boolean aced ) {
        try {
            String query;
            if (aced)
                query = "UPDATE tblDuel2Team SET fnWins = fnWins + 1, fnWinStreak = " + winnerStreak + 
    ...
    public void sql_storeTeamLoss(int teamID, int division, int loserStreak, int loserCurStreak, int lostAgainst, int rating, boolean aced ) {
        try {
            String query;
            if (aced)
                query = "UPDATE tblDuel2Team SET fnLosses = fnLosses + 1, fnLossStreak = " + loserStreak + 
    ...
    public ResultSet sql_getUserIPMID(String name) {
        ...
        try {
            String query = "SELECT p.fcIP, p.fnMID FROM tblDuel2Player p JOIN tblUser u ON u.fnUserID = p.fnUserID WHERE fcUserName = '" + Tools.addSlashesToString(name) + "'";
    ...
    public boolean sql_gameLimitReached( int a, int b, int division ) {
        try {
            String query = "SELECT COUNT(fnMatchID) AS count FROM `tblDuel2Match` WHERE"+
